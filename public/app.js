...
function renderCreatorCard(creator) {
  ...
  if (platforms.length > 0) {
    ...
    platforms.forEach((p, idx) => {
      ...
      chip.innerHTML = `<span class="meta-dot ${dotClass}"></span>${followerText}`;
...
function renderRosterTab() {
  ...
  renderListIncrementally(list, filtered, (creator) => renderCreatorCard(creator), { divider: false });
}
...
function scoreCreatorFilters(creator) {
  const totalFilters = dispatchFilters.niches.length +
...
function renderDispatchActiveStrip() {
  ...
  // Platform × Tier — shown in strip but excluded from match score count
  dispatchFilters.platformTiers.forEach(pt => {
    const label = `${pt.platform} ${TIER_SHORT[pt.tier] || pt.tier}`;
    addBadge('pt-' + pt.platform + '-' + pt.tier, label, 'platform', () => {
      dispatchFilters.platformTiers = dispatchFilters.platformTiers.filter(
        p => !(p.platform === pt.platform && p.tier === pt.tier)
      );
      renderDispatchFilters();
        ...
        <div class="marker-inner">
          ...
          <div class="marker-avatar-wrap">
            ...
            <div class="marker-avatar">
              ${creator.photo ? `<img src="${creator.photo}" alt="">` : `<span class="marker-initials">${getInitials(creator.firstName, creator.lastName)}</span>`}
            </div>
          </div>
...
function _arrangeMarkerRings() {
  ...
  // At wide zoom, markers are at their real locations — no grouping needed.
  const zoom = map.getZoom();
  if (zoom < 7) return;

  // Collect all visible markers with their pixel positions
  const entries = [];
...
function renderRing(creator) {
  ...
  // --- Navigation arrows for cycling through nearby creators ---
  // Only include creators visible in the current map viewport (+ small padding)
  const mapBounds = map.getBounds().pad(0.15); // 15% padding so edge pins are included
  const sortBy = document.getElementById('sortSelect') ? document.getElementById('sortSelect').value : 'newest';
  const allFiltered = getFilteredCreators('', sortBy, true).filter(c => c.lat && c.lng);
  const navCreators = allFiltered.filter(c =>
    mapBounds.contains(L.latLng(c.lat, c.lng))
  );
  // Sort by distance from current creator so cycling feels geographic
  const here = L.latLng(creator.lat, creator.lng);
  navCreators.sort((a, b) => {
    const distA = here.distanceTo(L.latLng(a.lat, a.lng));
    const distB = here.distanceTo(L.latLng(b.lat, b.lng));
    return distA - distB;
  });
  const currentIdx = navCreators.findIndex(c => c.id === creator.id);
  const n = navCreators.length;

  function cycleToCreator(targetCreator) {
    // Check if target is already visible on screen — if so, skip fly
    const targetPx = map.latLngToContainerPoint([targetCreator.lat, targetCreator.lng]);
    const mapSize = map.getSize();
    const inView = targetPx.x > -50 && targetPx.x < mapSize.x + 50 &&
                   targetPx.y > -50 && targetPx.y < mapSize.y + 50;

    // Close current ring instantly (no map restore)
    const ov = document.getElementById('ringOverlay');
    ov.classList.remove('open');
    ov.innerHTML = '';
    currentEditingCreator = targetCreator.id;

    if (inView) {
      // Nearby — instant re-render, no fly
      setTimeout(() => renderRing(targetCreator), 30);
    } else {
      // Far — smooth fly then render
      map.once('moveend', () => {
        setTimeout(() => renderRing(targetCreator), 60);
      });
      map.flyTo([targetCreator.lat, targetCreator.lng], Math.max(map.getZoom(), 10), {
        duration: 0.7,
        easeLinearity: 0.15
      });
    }
  ...
  if (navCreators.length > 1) {
    const navRow = document.createElement('div');
    navRow.className = 'ring-nav-row';

    const prevIdx = (currentIdx - 1 + n) % n;
    const nextIdx = (currentIdx + 1) % n;

    const prevBtn = document.createElement('button');
    prevBtn.className = 'ring-nav-btn';
    prevBtn.innerHTML = '‹';
    prevBtn.title = `Previous: ${getFullName(navCreators[prevIdx])}`;
    prevBtn.onclick = (e) => { e.stopPropagation(); cycleToCreator(navCreators[prevIdx]); };

    const counter = document.createElement('span');
    counter.className = 'ring-nav-counter';
    counter.textContent = `${currentIdx + 1} of ${n} nearby`;

    const nextBtn = document.createElement('button');
    nextBtn.className = 'ring-nav-btn';
    nextBtn.innerHTML = '›';
    nextBtn.title = `Next: ${getFullName(navCreators[nextIdx])}`;
    nextBtn.onclick = (e) => { e.stopPropagation(); cycleToCreator(navCreators[nextIdx]); };

    navRow.appendChild(prevBtn);
    navRow.appendChild(counter);
    navRow.appendChild(nextBtn);
    ringColumn.appendChild(navRow);
  }
  ...
  // Keyboard navigation — arrow keys to cycle, Escape to close
  function ringKeyHandler(e) {
    ...
    if (e.key === 'ArrowLeft' || e.key === 'ArrowUp') {
      e.preventDefault();
      const prevEl = overlay.querySelector('.ring-nav-btn:first-child:not(.disabled)');
      if (prevEl) prevEl.click();
    } else if (e.key === 'ArrowRight' || e.key === 'ArrowDown') {
      e.preventDefault();
      const nextEl = overlay.querySelector('.ring-nav-btn:last-child:not(.disabled)');
...
function _showNearestCompareCard(creator, distance, rankIndex, rankColors) {
  ...
  _hideNearestCompareCard(); // Remove any existing
...
function _hideNearestCompareCard() {
...
function renderNearestCreators() {
  ...
  nearest.forEach((item, i) => {
    ...
    // Fly-to + dashed arc + compare card on hover
    const RANK_COLORS = ['#c9a96e', '#a8a8a8', '#b87b5e'];
    let flyRestoreTimeout = null;
    row.addEventListener('mouseenter', () => {
      clearTimeout(flyRestoreTimeout);
      if (!c.lat || !c.lng || !dispatchDestination) return;
      // Save current view before flying
      if (!container._savedView) {
        container._savedView = { center: map.getCenter(), zoom: map.getZoom() };
      }
      ...
      // Show compare card if filters are active
    ...
    row.addEventListener('mouseleave', () => {
      ...
      // Remove compare card
      _hideNearestCompareCard();
      flyRestoreTimeout = setTimeout(() => {
        if (container._savedView) {
          map.flyTo(container._savedView.center, container._savedView.zoom, { duration: 0.4 });
          container._savedView = null;
        }
      }, 200);
    });
